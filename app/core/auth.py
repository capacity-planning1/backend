...
def create_student_access_token(student_id: UUID) -> str:
    now = datetime.now(timezone.utc)
    delta = timedelta(seconds=Settings.auth.access_token_lifetime_seconds)
    exp = now + delta
    payload = {
        'sub': str(student_id),
...
def create_refresh_token(
    student: StudentModel, expires_delta: timedelta | None = None
) -> str:
    now = datetime.now(timezone.utc)
    if expires_delta:
        exp = now + expires_delta
    else:
        exp = now + timedelta(seconds=settings.auth.refresh_token_lifetime_seconds)
